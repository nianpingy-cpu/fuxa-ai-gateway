...
describe('FuxaClient DAQ endpoints', () => {
  it('fetches historical data via GET /api/daq with a query', async () => {
    ...
    expect(calls[0]?.url).toContain('http://fuxa:1881/api/daq?');
    expect(calls[0]?.url).toContain('query=');
    expect(calls[0]?.url).toContain(encodeURIComponent('"sids":["t1"]'));
  });
});
